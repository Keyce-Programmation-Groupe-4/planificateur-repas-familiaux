...
function WeeklyPlannerPage() {
  ...
  const plannedMeals = weeklyPlanData ? 
    Object.values(weeklyPlanData.days || {}).reduce((count, day) => 
            ...
            <Fade in timeout={1000}>
              <Grid container spacing={{ xs: 2, sm: 3, md: 4 }}>
                {orderedDays.map((dayKey, index) => {
                  ...
                  return (
                    <Grid item xs={12} sm={6} md={4} lg={12/7} key={dayKey}>
                      <Zoom in timeout={400 + (index * 100)}>
                        <Box>
                          <DayColumn
                            dayKey={dayKey}
                            dayName={dayName}
                            date={new Date(currentWeekStart.getFullYear(), currentWeekStart.getMonth(), currentWeekStart.getDate() + orderedDays.indexOf(dayKey))}
